...
"""
train_gru.py
Train a GRU classifier on preprocessed RAVDESS features.

Usage:
    python train_gru.py --data ./data/ravdess_features_balanced.pt
"""
import os
import argparse
import random
...
def main(args):
    ...
    N, T, D = X.shape
    model = GRUClassifier(feat_dim=D, n_classes=8).to(DEVICE)
    crit = nn.CrossEntropyLoss()
    opt = torch.optim.Adam(model.parameters(), lr=args.lr)

    best_val = 0.0; best_path = os.path.join(args.out_dir, "gru_best.pt"); os.makedirs(args.out_dir, exist_ok=True)
    patience = 0
    ...
    for epoch in range(1, args.epochs+1):
        ...
        print(f"[GRU] Epoch {epoch} TrainAcc {tr_acc:.4f} ValAcc {val_acc:.4f}")
        if val_acc > best_val:
            best_val = val_acc
    ...
    print("GRU TEST Acc:", test_acc)
    torch.save({'state_dict': model.state_dict(), 'meta': meta}, os.path.join(args.out_dir, "gru.pt"))
